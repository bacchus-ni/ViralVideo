...
下面是一条完全由本系统渲染的能力演示片，每个镜头展示一种文字动效，节拍来自本地鼓点检测：

![纯文本动效能力演示](docs/videos/10-kinetic-showcase.gif)
...
- 本地鼓点检测出的节拍标记：检测成功时会覆盖模型估算值，且后续编辑分镜时长也不会被覆盖。
...
- 用【】括住的词会按当前强调样式渲染成变色、高亮块或描边。
- AI 生成的强调词会自动以【】形式显示在编辑框里。
- 删掉括号就是取消这个词的强调。
- 强调词必须是该行原文的连续子串，否则不会生效。
...
- 文字：单镜头文字色、强调色、强调词样式（变色、高亮块、描边）。
- 内容：标题副标题和字间注释镜头的副标题/竖排小字、原位换词镜头的换词组、爆发词镜头的爆发词、幽灵背景字。
- 运动：镜头时长、入场效果。
...
- 撞色过渡。
- 堆叠标题。
- 标题副标题：巨字标题加小号副标题，可叠幽灵背景大字。
- 原位换词：句架静止，强调位上的词随节拍原位替换。
- 爆发词：多个拟声词以随机角度、随机位置逐拍弹出。
- 字间注释：2~4 个大字拉开，竖排小字夹在字缝里。

支持的高级入场包括擦入、冲击、滑入、缩放、散开、打字和无入场，入场效果会真正参与渲染；打字入场在镜头内有节拍时会按节拍渐进补全。强调效果包括抖动、闪白、倾斜、裁切、描边、阵列和发光。单镜头还支持 zoom、pan、rotate 运镜，按镜头时长插值。

高级模板带有节拍轨（beatMarkers）：原位换词、打字补全、爆发词都会按落在该镜头内的节拍逐拍触发。从 demo 视频解析出的模板使用本地检测的真实鼓点，让动效真正踩在音乐上。
...
- 本地鼓点检测：解析 demo 时检测音频鼓点，覆盖模型估算的节拍。
- 槽位边界自动吸附到检测出的节拍。
...
- 文案使用完整编辑框。
- 文案每行映射为一个镜头。
- 文案编辑框支持【强调词】标记，行内强调支持变色、高亮块、描边三种样式。
- 分镜支持逐镜头编辑画面文字和说明。
- 分镜高级设置支持单镜头文字色、强调色、时长、入场、特效、背景。
- 高级模板镜头支持场景类型、入场方式、强调效果编辑。
- 标题副标题、原位换词、爆发词、字间注释四类高级场景，支持幽灵背景字和单镜头运镜。
- 原位换词、打字补全、爆发词按镜头内的节拍逐拍触发。
...
- 千问文生图 API
- FFmpeg，用于 demo 视频音频抽取、鼓点检测解码和渲染辅助
...
Remotion Studio 里除了默认的 TextMixComposition，还注册了两个对照合成：

- KineticFixtureMixcut1：对照 example_video 样片的目标效果，用于逐镜头比对调参。
- ShowcaseDemo：覆盖全部场景原语与节拍能力的演示片，即 README 开头的演示视频。

直接渲染演示片：

```bash
npx remotion render remotion/index.ts ShowcaseDemo public/renders/showcase-demo.mp4
```
...
lib/
  advanced-template.ts        # 高级模板归一化、槽位重排与节拍吸附
  beat-detect.ts              # 本地音频鼓点检测
  ...
  templates.ts                # 内置模板
remotion/
  fixtures/                   # 对照样片与能力演示合成，供 Studio 比对
...
- 鼓点检测失败（音频过短或节拍不明显）时会静默回退到模型估算的节拍，不阻塞解析。
